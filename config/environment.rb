...
ENV['RACK_ENV'] = RACK_ENV if ENV['RACK_ENV'] != RACK_ENV
ENV['NODE_ENV'] = NODE_ENV if ENV['NODE_ENV'] != NODE_ENV


# External url only applicable if you set NODE_ENV to external or development.
# This allows you to run the frontend from virtually anywhere.
FRONTEND_EXTERNAL_URL = ENV.fetch('FRONTEND_EXTERNAL_URL', 'http://127.0.0.1:8123')
